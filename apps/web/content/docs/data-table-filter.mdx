...
The `PropertyFilterOperator` and `PropertyFilterValue` components are internally represented by a `Controller`, which has two components:

- `Display` is the component rendering the visible UI of the controller.
- `Menu` is the component rendering the menu UI of the controller, including the trigger and content for the dropdown menu.
