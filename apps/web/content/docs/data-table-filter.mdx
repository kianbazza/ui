...
With the `server` strategy, you will need to specify the min/max values; *we can't compute them for you, since we don't have access to the full dataset*. For this case, we recommend setting the faceted `min` and `max` values when we create our instance (covered later on).
