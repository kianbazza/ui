...
```
#### Optional: Setting the locale

You can optionally specify a `locale` to localize the filter UI. Currently supported values are:
...
If no `locale` is provided, the component defaults to English.
