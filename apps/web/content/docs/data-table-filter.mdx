...
We have some examples that you can use as a reference to build your own applications.
...
Before we dive in, let's take a look at a basic scenario that will be referenced throughout the guides.
...
We can use the `min()` and `max()` methods to set the minimum and maximum values for a column.
...
We expose an `options` property on the instance, which is an object mapping column IDs to either:
  - a `ColumnOption[]`
  - a tuple `[ColumnOption[], Map<string, number>]`
  
The first is straightforward - pass in the options directly.

The tuple allows you to pass in both the options and a map of faceted unique values.

What do we mean by faceted unique values? Basically, it's a map of key-value pairs where each key is a column option, and the value is the number of times the option appear in the dataset.
...
> [!WARNING] When prompted (`y/N`), explicitly overwrite the `lib/i18n.ts` file.
...
- Overwrite the `lib/i18n.ts` file to add all supported locales.
- Add all supported locales to your project, under `locales/[locale].json`.
...
Then, extend the `Locales` type in `lib/i18n.ts` to include your new locale:
